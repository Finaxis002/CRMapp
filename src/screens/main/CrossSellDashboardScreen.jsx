// screens/main/CrossSellDashboardScreen.jsx — REFACTORED with UI Kit (header compacted)
...
const SectionLabel = ({ iconName, children }) => {
  ...
  return (
    <View style={sh.sectionLabel}>
      ...
      <Text style={[sh.sectionLabelText, { color: colors.textTertiary }]}>
        {children}
      </Text>
    </View>
  );
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        <View style={{ flex: 1, minWidth: 0 }}>
          <Text
            style={[sh.statLabel, { color: colors.textTertiary }]}
            numberOfLines={1}
          >
            {label}
          </Text>
          <Text
            style={[sh.statValue, { color: colors.textPrimary }]}
            numberOfLines={1}
          >
            {value}
          </Text>
        ...
        <View
          style={[
            sh.statIcon,
            { backgroundColor: color + '18', borderRadius: borderRadius.lg },
          ]}
        >
    ...
    <View style={sh.barRow}>
      ...
      <View style={{ flex: 1 }}>
        <View style={sh.barRowTop}>
          <Text
            style={[sh.barRowLabel, { color: colors.textSecondary }]}
            numberOfLines={1}
          >
            {label}
          </Text>
    ...
    >
      <View
        style={[
          ...
          selectedId === 'all' && { backgroundColor: colors.primarySoft },
        ]}
      >
        <TouchableOpacity
        ...
        >
          <Text
            style={[
              ...
              {
                color:
                  selectedId === 'all' ? colors.primary : colors.textPrimary,
      ...
      {users.map(u => (
        <View
          ...
          style={[
            sh.pickerOptionAll,
            { borderBottomColor: colors.border },
            selectedId === u._id && { backgroundColor: colors.primarySoft },
          ]}
        >
          <TouchableOpacity
            style={{ flexDirection: 'row', flex: 1, alignItems: 'center' }}
          ...
          >
            <Text
              style={[
                ...
                {
                  color:
                    selectedId === u._id ? colors.primary : colors.textPrimary,
                  fontWeight: selectedId === u._id ? '600' : '400',
                  flex: 1,
                },
              ]}
              numberOfLines={1}
      ...
      <View style={sh.modalOverlay}>
        ...
        >
          ...
          <View style={{ padding: spacing.lg }}>
            ...
            <View
              style={{ flexDirection: 'row', gap: 10, marginTop: spacing.xl }}
            >
              <ImprovedButton
                title="Cancel"
                variant="outline"
...
const AnalyticsTab = ({ currentUserId, isAdmin, isTL, isManager }) => {
  ...
  const load = useCallback(
    async (uid = selectedUserId) => {
      setLoading(true);
      try {
        const params = new URLSearchParams();
        if (!isAdmin && !isTL && !isManager && currentUserId)
          params.append('userId', currentUserId);
        else if ((isAdmin || isTL || isManager) && uid !== 'all')
  ...
  const interestedCount =
    (statusMap['Interested'] || 0) + (statusMap['Converted'] || 0);
  ...
  const selectedUserName =
    users.find(u => u._id === selectedUserId)?.name || 'All Users';
    ...
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      {(isAdmin || isTL || isManager) && (
        ...
        >
          ...
          <Text
            style={[sh.filterBtnText, { color: colors.textSecondary }]}
            numberOfLines={1}
          >
            {selectedUserName}
          </Text>
      ...
      <View style={sh.statGrid}>
        <StatCard
          ...
          label="Leads Approached"
          value={data.totalLeadsWithCrossSell}
          color="#2563eb"
        />
        <StatCard
      ...
      >
        ...
        <BarRow
          ...
          max={data.totalRecommendations || 1}
          color="#059669"
          iconName="check-circle"
        />
        <BarRow
        ...
        >
          <View style={sh.convRateCircle}>
            <Text style={[sh.convRatePct, { color: colors.textPrimary }]}>
              {data.conversionRate}%
            </Text>
          </View>
          <View style={{ flex: 1, minWidth: 0 }}>
            <Text style={[sh.convRateTitle, { color: colors.textPrimary }]}>
              Conversion Rate
            </Text>
            <Text style={[sh.convRateSub, { color: colors.textTertiary }]}>
              {interestedCount} of {data.totalRecommendations} accepted
            </Text>
            <View
              style={[
                sh.badge,
            ...
            >
              <Text style={[sh.badgeText, { color: colors.success }]}>
      ...
      >
        ...
        {data.topServices?.length > 0 ? (
          data.topServices.map(s => {
            ...
            return (
              <View key={s._id} style={sh.topSvcRow}>
                ...
                <Text
                  style={[sh.topSvcName, { color: colors.textPrimary }]}
                  numberOfLines={1}
                >
                  {s._id}
                </Text>
          ...
          <Text style={[sh.emptyText, { color: colors.textTertiary }]}>
            No data yet
          </Text>
        )}
      </ImprovedCard>
      ...
      {data.conversionByService?.length > 0 && (
        <ImprovedCard
          ...
          style={{ marginBottom: 12 }}
        >
          <SectionLabel iconName="trending-up">
            Conversion by Original Service
          </SectionLabel>
          {data.conversionByService.map(row => {
            const rate =
              row.total > 0
                ? ((row.interested / row.total) * 100).toFixed(0)
                : 0;
            const rColor =
              rate >= 50 ? '#059669' : rate >= 25 ? '#d97706' : '#dc2626';
            const meta = getSvcMeta(row._id);
            return (
              <View
                key={row._id}
                style={[sh.convRow, { borderColor: colors.border }]}
              >
                <View
                  style={[
                    sh.svcIconBox,
                ...
                <Text
                  style={[sh.convRowName, { color: colors.textPrimary }]}
                  numberOfLines={1}
                >
                  {row._id || '—'}
                </Text>
                <Text style={[sh.convRowMuted, { color: colors.textTertiary }]}>
                  {row.total} recs
                </Text>
                <View
                  style={[
      ...
      >
        ...
        {data.recentActivity?.length > 0 ? (
          data.recentActivity.map(rec => {
            ...
            const intCount =
              rec.recommendations?.filter(
                r => r.status === 'Interested' || r.status === 'Converted',
              ).length || 0;
            return (
              <View
                key={rec._id}
                style={[sh.activityRow, { borderColor: colors.border }]}
              >
                <Avatar name={name} size={34} rounded={17} variant="solid" />
                <View style={{ flex: 1, minWidth: 0 }}>
                  <Text
                    style={[sh.activityName, { color: colors.textPrimary }]}
                    numberOfLines={1}
                  >
                    {name}
                  </Text>
                  <Text
                    style={[sh.activitySub, { color: colors.textTertiary }]}
                    numberOfLines={1}
                  >
                    {product}
                  </Text>
                ...
                <View style={{ alignItems: 'flex-end' }}>
                  <Text
                    style={[sh.activityCount, { color: colors.textTertiary }]}
                  >
                    {recCount} recs
                  </Text>
                  {intCount > 0 && (
      ...
      <View style={{ flex: 1, flexDirection: 'row' }}>
        ...
        >
          <View style={[sh.drawerHeader, { borderBottomColor: colors.border }]}>
            <View style={{ flex: 1, minWidth: 0 }}>
              <Text
                style={[sh.drawerTitle, { color: colors.textPrimary }]}
                numberOfLines={1}
              >
                {lead?.name || ''}
              </Text>
              <Text
                style={[sh.drawerSub, { color: colors.textTertiary }]}
                numberOfLines={1}
              >
                {lead?.product || '—'}
              </Text>
            ...
            <TouchableOpacity
              onPress={onClose}
              style={[sh.drawerCloseBtn, { borderColor: colors.border }]}
            >
...
const LeadsOverviewTab = ({
  ...
  const load = async (p = 1, uid = selectedUserId, silent = false) => {
    ...
    try {
      ...
      else if (!isAdmin && !isTL && !isManager && currentUserId)
        params.append('userId', currentUserId);
      const res = await api.get(
        '/cross-sell/leads-overview?' + params.toString(),
      );
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      <View style={sh.tabHeader}>
        <View style={{ flex: 1, minWidth: 0 }}>
          <Text
            style={[sh.tabTitle, { color: colors.textPrimary }]}
            numberOfLines={1}
          >
            Leads Overview ({pagination.total})
          </Text>
          <Text style={[sh.tabSubtitle, { color: colors.textTertiary }]}>
            Cross-sell services and their status
          </Text>
        </View>
        {(isAdmin || isTL || isManager) && (
          <TouchableOpacity
            ...
            >
              <View style={sh.leadCardTop}>
                ...
                <View style={{ flex: 1, marginLeft: 10, minWidth: 0 }}>
                  <Text
                    style={[sh.leadName, { color: colors.textPrimary }]}
                    numberOfLines={1}
                  >
                    {name}
                  </Text>
                  <Text
                    style={[sh.leadPhone, { color: colors.textTertiary }]}
                    numberOfLines={1}
                  >
                    {lead?.phone || '—'}
                  </Text>
              ...
              <View style={{ flexDirection: 'row', gap: 8, marginBottom: 10 }}>
                ...
                >
                  <Text
                    style={[sh.infoChipLabel, { color: colors.textTertiary }]}
                  >
                    SERVICE
                  </Text>
                  <Text
                    style={[sh.infoChipValue, { color: colors.textPrimary }]}
                    numberOfLines={1}
                  >
                    {product}
                  </Text>
              ...
              {recs.length > 0 && (
                <View>
                  <Text style={[sh.recLabel, { color: colors.textTertiary }]}>
                    RECOMMENDATIONS
                  </Text>
                    ...
                    {recs.map((r, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <Text style={[sh.recChipStatus, { color: sc.color }]}>
                            {r.status}
                          </Text>
                        </TouchableOpacity>
                      );
      ...
      {pagination.totalPages > 1 && (
        <View style={sh.pagination}>
          <ImprovedButton
            ...
            disabled={page === 1}
          />
          <Text style={[sh.pageInfo, { color: colors.textTertiary }]}>
...
const RulesTab = ({ isAdmin, isTL, isManager }) => {
  ...
  const toggleActive = async rule => {
    try {
      ...
      setRules(p =>
        p.map(r => (r._id === rule._id ? { ...r, isActive: !r.isActive } : r)),
      );
    } catch {
      ...
      <View style={sh.tabHeader}>
        <View style={{ flex: 1, minWidth: 0 }}>
          <Text
            ...
            numberOfLines={1}
          >
            Service Rules ({rules.length})
          </Text>
          <Text
            style={[sh.tabSubtitle, { color: colors.textTertiary }]}
            numberOfLines={2}
          >
            Recommendations auto-appear when lead product matches
          </Text>
        </View>
        {(isAdmin || isTL || isManager) && (
          <ImprovedButton
          ...
          return (
            ...
            >
              ...
              >
                ...
                <Text
                  style={[sh.ruleName, { color: colors.textPrimary, flex: 1 }]}
                  numberOfLines={1}
                >
                  {rule.triggerService}
                </Text>
...
const CrossSellDashboardScreen = () => {
  ...
  const TABS = [
    ...
    ...(isAdmin || isTL || isManager
      ? [{ key: 'rules', label: 'Rules', icon: 'package' }]
      : []),
  ];

  return (
    <View style={[sh.container, { backgroundColor: colors.background }]}>
      {/* ── Slim header + tabs — ek hi surface block (PageHeader hata diya) ── */}
      <View
        style={[
          sh.headerBlock,
      ...
      >
        <Text
          style={[sh.headerTitle, { color: colors.textPrimary }]}
          numberOfLines={1}
        >
          Cross-Sell
        </Text>
        <Text
          style={[sh.headerSub, { color: colors.textSecondary }]}
          numberOfLines={1}
          ellipsizeMode="tail"
        >
          Manage services & track opportunities
        </Text>
      </View>
...
const sh = StyleSheet.create({
  ...
  filterBtnText: { fontSize: 12, fontWeight: '500', flexShrink: 1 },
